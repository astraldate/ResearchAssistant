...
- **状态：** 进行中
- 已恢复并保留阶段 15 与 1.1.13 发布状态，确认当前没有待回退的已跟踪改动。
- 已完成在线 PDF.js、离线 `react-native-pdf`、移动翻译接口和桌面连续阅读实现的只读对照。
- 已确认实施方向：默认连续滚动、可切单页、目录默认收起、会话级划词自动翻译、平板右侧译文栏和手机非模态底栏。
- 下一步先实现 Viewer v2 连续渲染与命令协议，再重构移动阅读壳层和自动翻译协调器。
- 已完成 Viewer v2 内核、移动协议纯函数和非模态译文停靠栏；阅读页首次整文件替换因绝对路径删除失败，原文件保持不变，将改用相对路径继续。
- 新建 `mobile-app/src/screens` 被 ACL 拒绝，路由已改为导出已有 `components` 目录中的新阅读页实现。
- 阅读器首轮并行门禁被 Node 测试的 `spawn EPERM` 环境限制中断；代码测试尚未执行，将受控重跑并分别保留其他门禁输出。
- 移动 TypeScript、PDF 阅读器纯逻辑测试 4/4 和 Rust 格式检查已通过；定点 Prettier 发现 4 个机械格式差异，正在按仓库标准修正。
- Viewer Rust 定点测试与 `cargo check` 已通过；版本统一补丁出现两处先行更新，已核对并补齐其余 1.1.14 与 Android versionCode 24 字段。
- 双端 TypeScript、移动纯逻辑测试 9/9、Rust 格式与 52/52 单元测试已通过；全仓 Prettier 只剩 4 个版本生成 Schema 的机械差异。
- 全仓 Prettier 已复检通过；冲突标记脚本因沙箱 `spawnSync git EPERM` 未执行完成，将在受控环境重跑。
- 冲突标记、版本一致性和差异空白检查已通过；Web 生产构建因 Esbuild 沙箱派生进程限制未完成，将受控重跑。
- Web 1.1.14 生产构建已在受控环境通过，仅保留既有 PDF.js eval、动态导入和大分块提示。
- 阶段 16 功能实现与静态门禁已完成，正在构建 Windows NSIS 与 Android Release 正式产物。
- Windows NSIS 1.1.14 正式安装包构建成功，文件大小为 54,210,181 字节，ProductVersion 与 FileVersion 均核验为 1.1.14。
- Android Release APK 构建成功，文件大小为 109,508,206 字节，核验 `versionName 1.1.14`、`versionCode 24` 与 APK v2 签名有效。
- 首次复制 APK 到受保护分发目录因 ACL 拒绝；核验源文件和目标路径后按权限流程受控重试，已更新 1.1.14 版本化文件与稳定文件。
- 按用户要求未计算 SHA-256；未下载模型、未执行 OCR 推理、未调用 Ollama。
- 阶段 16 功能、版本、门禁与正式产物均已完成；真机视觉验收保留为交付后的设备侧确认，不阻塞本次构建交付。
- 未执行 Git 提交、标签、远端推送或 GitHub Release，等待用户提供对应远端发布授权。
- 正式构建重新生成 4 个 Tauri Schema 后出现机械格式差异，已连同阶段计划按仓库 Prettier 规则格式化。
- 最终轻量复检通过：全仓 Prettier、冲突标记、版本一致性和 `git diff --check` 均成功；Git 仅提示 Android Gradle 工作副本未来会将 CRLF 规范化为 LF，不影响本次结果。

### 阶段 17：移动端 PDF 选区稳定性修复

- **状态：** 完成
- 用户真机反馈选中文本后选区仍会突然拓展；本轮保留全部未提交内容，只修复 PDF.js 文字层选区链路。
- 已确定先检查 Range 端点归一化、触控手柄、连续页面渲染窗口和异步 selectionchange 回写，再以回归测试固定边界。
- 已定位两条确定性根因：端点轻点被误判为拖动，以及邻栏更近时仍强制映射到原栏远端字符。
- 用户截图进一步确认同栏跳行：右栏首行与下方段落被归入同一流，中间行却落入其他流；将同步修正阅读流栏带合并。
- 仓库中没有第二套可复用的移动视觉选区算法；现有 Rust 测试通过检查内嵌 Viewer 行为标记做回归，因此本轮将在同一测试中补充端点偏移、跨栏冻结和同栏流合并断言。
- 已实现稳定栏带合并：同栏的分段阅读流按水平重叠、中心距离和左右边界重新聚合，宽度超过页面 72% 的跨栏内容保持隔离。
- 已修复端点手柄：轻点不重算边界，拖动扣除抓取偏移，触屏移动超过 10px 才进入拖动；邻栏明显更近时直接冻结当前选区。
- 已在 Rust Viewer 回归测试中增加同栏合并、跨栏冻结、抓取偏移、触屏阈值和轻点不拖动断言。
- Viewer JavaScript 语法检查通过，Rust 定点测试 1/1 通过；`cargo fmt --check` 仅发现一处长断言换行差异，已执行标准格式化修正。
- 新截图确认还存在 Android WebView 原生选区问题：左侧虚空块是换行节点高亮，段末拓展是原生 Range 的 DOM 阅读顺序漂移；本轮范围扩展为同时修复原生与自定义选区。
- 已确认根因是宿主将 Viewer 选字模式与自动翻译开关绑定；将改为 v2 Viewer 始终使用受控视觉选区，自动翻译仅控制请求时机。
- 用户补充划词模式会在 PDF 空白吸附最近单词并阻断翻页；已定位为无限命中距离、`touch-action:none` 和按下即拦截三项共同作用，将加入空白命中门限与触屏滚动优先分类。
- 已将 v2 宿主改为始终启用受控视觉选区，自动翻译开关不再关闭 Viewer 选字模式；向前或向后扩展的 Android 原生 Range 路径因此不再参与新客户端交互。
- 已增加字符命中半径和触屏候选状态：空白轻点不吸附，正文纵向移动超过 10px 即取消候选并保留滚动，轻点字符才生成单词选区；端点手柄仍使用即时捕获。
- 已补 PDF.js `br::selection` 透明规则，旧客户端或降级原生选区不再显示每行最左侧的虚空高亮块。
- Viewer JavaScript 语法、双栏同栏连续性、跨栏隔离和滚动候选门禁合成检查通过。
- 移动 TypeScript、Rust 格式、Rust 编译和 Viewer 定点测试通过；移动 PDF 纯逻辑测试增至 5/5，通过 v2 受控选区策略回归。
- 最终门禁通过：Rust 52/52、移动 PDF 5/5、移动 TypeScript、Viewer JavaScript 语法、Rust 编译、全仓 Prettier、冲突标记与 `git diff --check` 均成功。
- 本轮未构建或覆盖 Windows/Android 安装产物；需要真机验证空白滚动、正文轻点选词、端点拖动以及双栏跨段边界。
- 用户指出没有新 APK 无法进行真机验证；阶段 17 重新进入构建状态，开始重建 Android Release 并更新分发文件。
- 已确认 Android 构建配置仍为 `versionName 1.1.14`、`versionCode 24`，Release 优先使用本地 keystore，缺失时按项目既有策略使用测试签名；现有分发 APK 是修复前产物，将由新构建覆盖。
- Android Release 构建成功，Gradle 542 个任务完成；新 APK 为 109,508,282 字节，构建时间晚于修复前产物。
- 构建目录 APK 核验为 `versionName 1.1.14`、`versionCode 24`，APK Signature Scheme v2 有效，签名者数量为 1。
- 首次覆盖受保护分发目录因 ACL 拒绝；核验源与目标均位于工作区后受控重试成功，版本化与稳定 APK 均已更新。
- 两个分发 APK 均再次通过版本与 APK v2 签名核验，可用于真机安装测试；按用户要求未计算 SHA-256。

### 阶段 18：移动端 PDF 真机选区回归修复

- **状态：** 进行中
- 阶段 17 的真机 APK 未通过用户验收：空白触摸仍吸附文字并阻断滚动，选区仍会跨到右栏。
- 已重新打开选区稳定性阶段，计划以视觉行为最小单元重建栏聚类，并限制触屏只能命中可见字形、滑动优先。
- 用户明确否决长按方案；尚未构建的长按实现已撤回，保持轻点文字直接选中。
- 首轮并行检查被一处 Viewer 回归断言的 `rustfmt` 换行差异中断；已执行项目标准格式化，后续检查改为分别回收结果。
- 已完成第一轮代码修复：过滤全部空白字符、触摸只接受实际文字 `span` 内的字符覆盖命中、8px 移动取消候选、滚动与模式切换清理候选。
- 已从所有上游流抽取并按视觉行 ID 去重，使用稳健的左右边界中位数重新聚类栏带；宽度达到页面 72% 的跨栏行保持独立。
- Viewer JavaScript 语法、污染流拆栏合成验证、Rust 格式和 Viewer 定点测试 1/1 均通过。
- 完整门禁首轮中 Rust 编译、双端 TypeScript 通过；Rust 全库 51/52，唯一失败为删除重复 CSS 后结构断言过严，已改为检查规则前缀。
- 移动 PDF Node 测试未进入用例执行，受限环境派生进程时报已知 `spawn EPERM`；将按权限流程受控重跑。
- 修正结构断言后 Rust 格式、Rust 编译和 Rust 全库 52/52 通过；移动 PDF 测试在受控环境 5/5 通过。
- 桌面与移动 TypeScript 均通过；移动 Node 仅保留既有的包类型重解析性能提示，不影响功能。
- 收尾首轮中版本一致性、差异空白和 Viewer 语法通过；三份中文计划文件存在 Prettier 机械差异，冲突脚本在沙箱内调用 `git` 被 `spawnSync EPERM` 拒绝。
- 三份中文计划文件机械格式化后，全仓 Prettier、受控冲突标记、差异空白和 Rust 格式复检全部通过。
- 阶段 18 代码门禁已完成，开始重建 Android Release APK。
- Android Release 构建成功，542 个任务完成；构建目录 APK 为 109,508,282 字节。
- 复核部署链路发现选区 Viewer 由桌面 Rust 服务提供，单独更新 APK 不会更新该算法；本轮增加 Windows NSIS 重建，真机验收时必须同步更新并重启桌面端。
- APK 核验为 `versionName 1.1.14`、`versionCode 24`，APK Signature Scheme v2 有效且签名者数量为 1。
- Windows NSIS 首轮在沙箱启动 Esbuild 时出现已知 `spawn EPERM`，将按权限流程受控重跑。
- Windows NSIS 在受控环境构建成功，正式安装器包含新的桌面 Rust Viewer。
- 普通权限覆盖两份 APK 分发文件被 ACL 拒绝；源与目标均已核验位于工作区，将受控覆盖两个明确文件。
- 两份 APK 分发文件已受控覆盖，均核验为 `versionName 1.1.14`、`versionCode 24`、APK v2 签名有效且签名者数量为 1。
- Windows NSIS 安装器为 54,227,837 字节，ProductVersion 与 FileVersion 均为 1.1.14。
- 已确认 Viewer HTTP 响应使用 `cache-control: no-store`，移动 WebView 关闭缓存并使用隐私会话；更新并重启桌面端后不会继续使用旧 Viewer。
- 阶段 18 完成；真机验证必须先安装并启动新的 Windows 桌面端，再连接移动端测试空白滑动、轻点文字和双栏选区。
- 本轮完成代码与测试后必须重新构建 Android Release APK，不再只交付源码验证结果。

### 阶段 19：移动端 Viewer 修订固化与 APK 真实重建

- **状态：** 进行中
- 已纠正归属说明：选区 Bug 在 Android WebView 中发生，不能把代码由桌面路由分发等同于桌面交互问题。
- 将在 `mobile-app` 中加入可测试的 Viewer 修订 URL 与 WebView 新会话加载标识，使 Android 打包输入真实变化后重新构建 APK。
- 首次检索移动 URL 定义的工具编排脚本存在语法错误，未执行任何命令；已记录并改用完整调用。
- Windows 下直接传入 `**/*.test.ts` 被 `rg` 当作非法路径；后续改为目录配合 `-g` 文件过滤。
- 已在移动端加入 Viewer 修订 URL、WebView 新会话 `key` 和严格字形能力门禁；旧 Viewer 不再进入有缺陷的在线选区路径。
- 桌面 Viewer 协议升级为 v3 并声明 `strict-glyph-selection`，新旧两端的行为边界可由握手明确判断。
- 移动 TypeScript、移动 PDF 测试 6/6、Rust 格式、Rust Viewer 定点测试和 Viewer JavaScript 语法均通过。
- 移动源码已经真实变化，开始重新执行 Android Release 构建；本次 Gradle 必须重新生成 JS Bundle 与 APK，而不是 `UP-TO-DATE` 复用旧包。
- Android Release 真实重建成功，38 个任务重新执行；新 APK 时间为 2026-08-15 22:22:42，大小为 109,508,770 字节，与 21:20 的旧包不同。
- 新 APK 核验为 `versionName 1.1.14`、`versionCode 24`，APK v2 签名有效且签名者数量为 1。
- 两份 APK 分发文件已覆盖为 22:22 的新移动构建；配套 v3 Windows NSIS 于 22:26 构建成功。
- 新 Windows 安装器为 54,222,556 字节，ProductVersion 与 FileVersion 均为 1.1.14。
- 最终门禁通过：移动 TypeScript、移动 PDF 6/6、Rust Viewer 定点测试、Viewer 语法、Rust 格式、全仓 Prettier、冲突标记、版本一致性和差异空白均成功。
- 阶段 19 完成；本次 APK 是包含移动端 Viewer 修订与兼容门禁的真实新构建，不再复用 21:20 的旧二进制。

### 阶段 20：双栏污染行拆分与跨行选区修复

- **状态：** 进行中
- 用户新截图确认单个视觉行已经横跨左右栏，因此选区会整行跨栏且无法连接同栏下一行。
- 将在稳定栏聚类前二次按字符栏沟拆分 `run`，并用视觉 `lineId` 维持同栏跨行语义。
- 已实现污染宽行二次拆分、字符 `lineId` 和同栏跨行文本重建；移动 Viewer 修订号升级为 `strict-glyph-selection-v2`，确保新 APK 开启全新会话。
- Viewer 语法、单个污染宽行拆栏合成验证、Rust 格式和 Viewer 定点测试均通过；合成结果确认左右栏完全隔离且左栏可继续到下一行。
- Rust 编译、Rust 全库 52/52、移动 TypeScript 和移动 PDF 6/6 全部通过。
- 全仓 Prettier、冲突标记、版本一致性、差异空白和 Rust 格式复检通过，开始重建阶段 20 双端正式产物。
- Android Release 与 Windows NSIS 均构建成功，Android 38 个任务重新执行，未复用旧 APK。
- 阶段 20 APK 已覆盖分发目录：时间为 2026-08-15 22:42:12，大小为 109,508,770 字节，`versionName 1.1.14`、`versionCode 24`、APK v2 签名有效。
- 配套 Windows NSIS 时间为 2026-08-15 22:45:02，大小为 54,227,435 字节，ProductVersion 与 FileVersion 均为 1.1.14。
- 阶段 20 完成；真机重点验收左栏跨行连续选择、栏沟隔离和右栏不进入选区。
- 最终定点 Prettier 命令误包含 Rust 文件，因无法推断 `.rs` 解析器退出；Rust 格式已独立通过，排除该文件后重新检查。

### 阶段 21：空白轻点取消选区闭环

- **状态：** 进行中
- 用户在发布前补充移动 PDF 交互要求：轻点空白处应清除当前选区并让移动端选中文字状态失效。
- 已暂停发布流程并恢复文件化计划；当前全部未提交和未跟踪内容保持不变。
- 实施边界确定为“空白轻点清除、空白滑动不清除”，不改变文字轻点选词和端点拖动行为。
- 已定位 Viewer 事件链：两个空白分支当前直接返回；`pointermove` 已具备 8px 候选取消阈值，可直接复用于空白滑动保护。
- 已确认移动宿主现有空 `selection` 处理会清空 `selectedText` 和 `selectedContext`，且空文本不会进入自动翻译队列。
- 已确认自定义高亮不拦截指针，端点手柄分支可保持不变；准备修改空白候选、抬起清除消息和 Viewer 修订号。
- 已实现空白延迟候选：文字层空白和未命中字形都记录 `blank`，移动超过 8px 只取消候选，阈值内抬起才调用 `clearSelection()` 并发送空 `selection`。
- 移动宿主收到空选区后清空文字与 Context，并使当前自动翻译队列的待处理项和活动键失效，旧异步结果不会在清除后回写。
- 移动 Viewer 修订号已升级为 `strict-glyph-selection-v3`，并增加空选区解析与 Rust Viewer 结构回归断言。

### 阶段 22：第七篇论文移动同步诊断与修复

- **状态：** 进行中
- 用户反馈桌面端已有 7 篇论文，但最新一篇无法同步到移动端；阶段 21 的源码修复保持不变，发布与构建继续暂停。
- 将优先检查是否残留演示期 6 条上限、服务端排序/分页错误、移动端缓存未刷新或新论文状态过滤。
- 已定位根因：移动接口仍读取 `demo_library.json` 并按 6 篇演示白名单过滤，新论文不在 `visible_file_names` 中，因此不会进入移动论文页和聊天 `@` 选择器。
- 正在核对同一过滤器对论文下载和聊天证据的影响，确保一次移除遗留限制后移动功能完整一致。
- 已确认 PDF 下载路径没有白名单，但工作区创新回退仍受其限制；将移除运行时演示过滤及其默认 6 篇常量，不删除既有配置文件。
- 已移除移动论文列表和工作区创新回退的演示白名单过滤，同时删除默认 6 篇常量与对应旧测试；既有 `demo_library.json` 不会被删除，只是不再读取。
- 风险门禁指出全量工作区同步可能扩大移动端可见数据范围，已先恢复原白名单实现；用户随后明确比赛演示结束并授权全部工作区同步，现按实用模式继续实施。
- 已按授权正式移除 `demo_library.json` 运行时读取、默认 6 篇常量及所有路径过滤；移动列表和工作区创新回退现在使用全部当前 PDF。
- README 与 1.1.14 变更记录已切换为实用同步语义，并增加生产代码不再引用演示白名单的 Rust 回归测试。
- 首轮并行定点门禁被移动 Node 测试的已知沙箱 `spawn EPERM` 中断，尚不代表用例失败；将按受控终端重跑并分别收集 Rust、TypeScript 结果。
- 受控移动 PDF 纯逻辑测试 7/7 通过，新增空选区消息用例成功。
- Rust 格式、移动论文全量同步定点测试、移动 PDF Viewer 定点测试和移动 TypeScript 均通过。
- Viewer JavaScript 语法、`cargo check`、Web TypeScript 和版本一致性通过。
- 全仓 Prettier 仅发现 `task_plan.md` 机械格式差异；冲突标记脚本在沙箱遇到已知 `spawnSync git EPERM`，将分别格式化并受控重跑。
- Rust 全库 52/52、全仓 Prettier、受控冲突检查、版本一致性、Web 与移动 TypeScript、差异空白检查全部通过。
- 阶段 21 与阶段 22 的源码和质量门禁完成，开始并行重建 Android Release APK 与 Windows NSIS。
- Android Release 构建成功，542 个任务中 38 个重新执行；Windows 构建命令因 PowerShell 可执行路径含空格且缺少调用运算符而未启动，将单独修正命令重跑。
- Windows NSIS 重跑成功；新 APK 为 109,508,870 字节，`versionName 1.1.14`、`versionCode 24`、APK v2 签名有效；新 NSIS 为 54,282,317 字节，ProductVersion 与 FileVersion 均为 1.1.14。
- 普通权限覆盖两个 APK 分发文件被 ACL 拒绝，已核验源和目标均在工作区，将按权限流程受控重试。
