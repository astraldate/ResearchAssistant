...
export function CapturePanel() {
  ...
  const handlePickAsset = async (source: "camera" | "library") => {
    ...
    if (!result.canceled) setPickedAsset(result.assets[0]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const assetBase64 = pickedAsset?.uri
        ...
        : null;
      const response = await submitInboxItem(
        session.baseUrl,
        session.deviceToken,
        {
          captureKind: pickedAsset ? "image" : url.trim() ? "url" : "note",
      ...
      <View style={styles.panel}>
        ...
        <View style={styles.actionRow}>
          <Pressable
            ...
            onPress={() => void handlePickAsset("camera")}
          >
            <Text style={styles.secondaryButtonText}>拍照</Text>
          </Pressable>
          <Pressable
            style={styles.secondaryButton}
            onPress={() => void handlePickAsset("library")}
          >
        ...
        {pickedAsset ? (
          <Text style={styles.assetText} numberOfLines={2}>
            已选图片：{pickedAsset.fileName || pickedAsset.uri}
          </Text>
        ) : null}
        <Pressable
        ...
        >
          {isSubmitting ? (
            ...
            <Text style={styles.primaryButtonText}>发送到桌面收件箱</Text>
          )}
        </Pressable>
      </View>
...
const styles = StyleSheet.create({
  ...
  secondaryButtonText: { color: palette.secondary, fontWeight: "800" },
  assetText: { color: palette.slate, fontSize: 13 },
  primaryButton: {
  ...
  statusTitle: { color: palette.secondary, fontWeight: "800" },
  statusText: { color: palette.ink, lineHeight: 22 },
});
