...
- 在线与离线模式默认纵向连续滚动，并保留单页模式；阅读页使用专用全屏安全区壳层，不再通过通用标题和固定平板工具区压缩 PDF。
- 目录默认收起。在线目录读取 PDF Outline，离线目录读取 `react-native-pdf` 的 `tableContents`；无目录时降级为页码列表。
...
- PDF.js 使用 `256 KiB` range chunk，关闭自动抓取和流式整文件预取；连续模式创建轻量页面占位，只渲染当前页前后两页并释放离开窗口的 Canvas 与文字层。
- 移动端先查找已有缓存，不在在线阅读器首屏阶段并发下载整份 PDF；阅读器 ready 或进入离线回退后再启动后台缓存。
- Token 仅存在于请求头和 WebView 页面内存，不进入 URL、日志或 PDF 缓存。

#### 文字选择与 AI 操作

- 默认保留系统长按选词；工具栏另提供无需长按的“划词翻译”模式。该模式在当前阅读会话开启一次后，每个稳定选区自动调用现有划词翻译接口。
...
- WebView v2 继续发送 `ready`、`page`、`selection` 和 `error`，并增加协议版本、能力、目录、阅读模式与交互消息；原生端通过受控命令执行跳页、连续/单页切换、选字模式、适宽和清理选区。移动端提供：
  - 翻译选中内容
  - 解释术语
  - 翻译当前页
  - 保存解释知识卡片
- 选区翻译最多 2400 个 Unicode 字符，术语最多 120 个 Unicode 字符；页码必须在文件实际范围内。
- 自动划词翻译采用单请求加最新选区队列；旧结果不得覆盖新选区，重复页码与文字组合直接读取会话缓存。
...
- PDF.js 按 Range 加载当前页邻域并连续滚动，平板关闭译文栏后 PDF 占满主区；桌面断开后已缓存 PDF 可保持页码并切换离线阅读
